...
Deployed on Streamlit Community Cloud: point the app's **Main file path**
at `app/app.py`. Dependencies are read from the `requirements.txt` at the
repository root (a copy of `app/requirements.txt`). The app loads its
fitted parameters from `app/models/app_params.json` and the data snapshot
from `app/data/bw_series.csv`, so it needs neither `agridatasets` nor any
model fitting at runtime.
